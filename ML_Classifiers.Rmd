...
```
- As we can observe from the graph above. The default number of trees in R is 500 and after 500 trees, the error seems to stabilize. 
...
predictions <- predict(forest.adni.sensitivity, newdata = test, type = "response")
predictions <- as.numeric(as.character(predictions))  # Convert to numeric if needed
predictions_binary <- ifelse(predictions > 0.5, 1, 0)
accuracy <- predictions_binary == test$converter
accuracy_rate <- sum(accuracy) / nrow(test)
...
```
- As we can we see above, the accuracy is a bit higher than when we only included completers in the data set. 
